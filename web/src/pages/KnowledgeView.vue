...
import type { DocumentItem, KnowledgeBase, SearchResult } from '../types'
...
async function createKnowledgeBase(): Promise<void> {
  ...
  try {
    const created = await request<KnowledgeBase>('/knowledge-bases', {
      method: 'POST',
      ...jsonBody({ name: createForm.name.trim(), description: createForm.description.trim() }),
    })
...
async function search(): Promise<void> {
  ...
  try {
    const data = await request<{ items: SearchResult[] } | SearchResult[]>(`/knowledge-bases/${selectedId.value}/search`, {
      method: 'POST',
      ...jsonBody({ query: searchQuery.value.trim(), top_k: 5 }),
    })
      ...
      <div v-else class="knowledge-layout">
        ...
        <div v-if="selected" class="kb-content">
          ...
          <section class="panel panel-padded search-panel">
            <div class="section-heading"><div><h2>检索试验台</h2><p>直接检查 Top-5 召回片段，不调用大语言模型</p></div></div>
            ...
            <div v-else-if="searchResults.length" class="result-list">
              <article v-for="(result, index) in searchResults" :key="`${result.document_id}-${result.chunk_index}`" class="result-card">
                ...
                <div>
                  <header><strong>{{ result.filename }}</strong><span>块 {{ result.chunk_index }}<template v-if="result.page"> · 第 {{ result.page }} 页</template></span><em>{{ (result.score * 100).toFixed(1) }}%</em></header>
                  <p>{{ result.content }}</p>
                </div>
              </article>
            </div>
