...
- 九个原创 Skills，包括知识库问答、分析写作、`find-skills` 和 `skill-creator`；支持 ClawHub 与 GitHub 白名单目录，远程安装经过版本锁定、双重安全扫描和人工确认。
- 每用户 Project 工作空间统一管理 Agent、知识库、会话与 Run；既有资源自动迁移到“默认项目”。
- Project 支持一次选择多个智能体；Agent 与知识库均保持项目级归属和历史快照。
- “内容生成”工作台可直接生成图片、Markdown/DOCX/PDF 文档、PPTX 和已结束 Run 的智能体运行
  报告；对话中的智能体仍可在人工审批后生成并归档制品。
- PDF 采用文字层优先、扫描页 PaddleOCR 回退，记录文字/OCR/混合提取方式和 OCR 页码。
- 中文提示词模板支持分类检索、复制编辑并快速应用到 Agent 系统提示词。
- 内置“数字人形象设计师”将文字描述本地映射为动态 2D 卡通形象，支持眨眼、呼吸、暂停和 SVG 导出。
- Agent 配置版本与恢复、最近 100 次 Run 的状态/耗时/工具/RAG Trace 汇总。
- 模型端点、Agent、知识库、MCP、Skills、内容生成、运行监测与系统状态的中文 Web 管理界面；
  数字人工作台位于“设置”中。
...
再次运行 `start.command` 时会检测前端、后端和 `.env` 是否比当前构建或进程更新，并按需自动
重建或重启，避免继续复用缺少新增 API 的旧服务。若修改 `.env` 中的 `HI_AGENT_PORT`，启动和
诊断脚本会使用新端口。

## Windows 安装与启动

支持 Windows 10/11 x64 和 Windows PowerShell 5.1 或 PowerShell 7。需要约 2 GB 可用空间、
网络连接和解压 ZIP 的系统能力；不要求预装 Python、Node.js、pnpm，也不要求管理员权限。
安装脚本会把 uv、Python 3.12、Node.js 22.17、pnpm、Playwright Chromium 和项目依赖安装到
仓库自己的 `.tools\` 与虚拟环境中，不修改系统 PATH。Windows ARM64 尚未完成原生依赖验证。
...
浏览器会自动打开 <http://127.0.0.1:8787>。停止和诊断命令为：
...
`-ExecutionPolicy Bypass` 只对本次 PowerShell 进程生效，不会更改系统执行策略。网络受限时可先跳过
体积较大的浏览器或嵌入模型下载，之后重新运行完整安装即可补齐：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\bootstrap.ps1 `
  -SkipBrowserDownload -SkipModelDownload
```

Windows 启动器与 macOS 启动器行为一致：源码、前端或 `.env` 更新后会按需重建/重启，运行数据仍
保存在 `data\`，日志写入 `logs\hi-agent.log` 和 `logs\hi-agent.error.log`。若公司代理拦截
`astral.sh`、`nodejs.org`、PyPI、npm 或模型仓库，需要先按组织策略配置 PowerShell、uv、Node 的
HTTPS 代理；安装脚本不会关闭 TLS 校验。

模型配置方式如下。Windows 本机不运行仓库内的 Linux/NVIDIA vLLM Compose；可连接局域网
或本机其他 OpenAI-compatible `/v1` 服务。若希望在同一台 Windows 电脑上运行 NVIDIA vLLM，
请在满足驱动和容器要求的 WSL2 Linux 环境中按后文 vLLM 流程部署。

模型可在 Web 的“设置 → 模型与系统”页面创建，也可通过 `.env` 初始化：
...
默认虚拟账号:13800000000 输入后生成验证码即可登录
...
RAG 问答完成后可由服务端按 Run 快照导出 Markdown、Word DOCX 或嵌入中文字体的 PDF，
报告包含问题、回答、执行流程、知识库、生成时间和可核验引用，不在服务端长期留存副本。

<img width="2816" height="1320" alt="17727edb-42f6-478b-a5be-a7fc2b85f128" src="https://github.com/user-attachments/assets/0d74e173-1609-45d7-b50b-a5e31964a733" />


## MCP

首次启动会创建一个默认关闭的 `workspace` stdio 示例配置，指向安装后的
`mcp_servers/.venv/bin/hi-agent-mcp`（Windows 为
`mcp_servers\.venv\Scripts\hi-agent-mcp.exe`）。在“MCP 服务”页面启用、探测工具，再把它绑定到
Agent 即可使用。项目还会以默认关闭状态预置 Filesystem、Git、Fetch、Time、Memory、
Sequential Thinking 和 GitHub Read-only；第三方包固定版本且只在主动探测或调用时下载运行。
配置与安全说明见 [MCP 预置服务](docs/MCP_PRESETS.md)，原始示例见
[mcp_servers/config.example.json](mcp_servers/config.example.json)。
...
“远程发现”同时接入 ClawHub 官方 HTTP API 与 `.env` 中
`HI_AGENT_REMOTE_SKILL_CATALOGS` 允许的 GitHub 仓库。ClawHub 安装固定精确版本，要求
官方 `verify` 返回 `pass/clean`，校验制品 SHA-256 后再通过本地路径与危险内容扫描；
来源、发布者、版本、哈希和安装时间记录在 Skill 内。发现与安装阶段绝不执行远程脚本，
安装后仍需手动绑定到 Agent。可将
...
macOS、Windows 原生环境和无 CUDA 主机不启动 vLLM。请在安装 Docker、NVIDIA Container
...
REST 统一前缀为 `/api/v1`，覆盖 `/auth`、`/projects`、`/models`、`/agents`、`/knowledge-bases`、
`/documents`、`/mcp/servers`、`/skills`、`/skills-remote`、`/sessions`、`/runs`、
`/observability`、`/digital-humans` 和 `/system/status`。
...
RAG 报告接口为 `POST /rag/reports`，使用当前用户拥有的已完成 `run_id` 和
`md`、`docx` 或 `pdf` 格式参数。
...
Windows 使用 `powershell -ExecutionPolicy Bypass -File .\scripts\doctor.ps1` 执行等价环境诊断。

离线单元测试使用 fake LLM 和确定性嵌入，不产生模型 API 费用。Docker Compose 在 macOS 和
Windows 仅做配置验收；真实 vLLM 启动与推理需要 Linux/NVIDIA 主机。
...
- [AGENTS.md](AGENTS.md)：编码 Agent 的仓库规则、验证命令和完成标准。
- [架构说明](docs/ARCHITECTURE.md)：组件边界、Agent 执行链路和关键不变量。
- [动态数字人设计](docs/DIGITAL_HUMAN.md)：2D/3D 调研、当前参数契约、安全边界和升级方向。
- [贡献指南](CONTRIBUTING.md)：开发流程、代码规范和测试要求。
- [安全策略](SECURITY.md)：漏洞报告方式及认证、凭据、工具和部署边界。
- [变更记录](CHANGELOG.md)：已发布与待发布的重要变化。
